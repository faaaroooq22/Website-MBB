  ...
  return (
    <div className="min-h-screen bg-[#111111] pb-32 relative">
       ...
       <div className="sticky top-0 z-40 bg-gray-950/90 backdrop-blur-md border-b border-gray-800 shadow-lg px-4 py-3 flex items-center justify-between">
           <button 
             onClick={onBack}
             className="p-2 bg-gray-900 border border-gray-700 rounded-full hover:bg-gray-800 hover:border-red-600 transition-all group"
           >
               <ArrowLeft className="w-5 h-5 text-gray-300 group-hover:text-red-600 transition-colors" />
           </button>
           ...
           <div className="bg-gray-900 px-3 py-1.5 rounded-lg border border-red-900/50 min-w-[80px] text-center">
               ...
               <span className="font-bebas text-lg text-red-600">{totalPrice()}/-</span>
           </div>
       </div>
       ...
       <div className="max-w-2xl mx-auto px-4 py-8 space-y-8 relative z-10">
            ...
            <section className="bg-gray-900/50 border border-gray-800 p-5 rounded-2xl animate-in slide-in-from-bottom-4 duration-500">
                <div className="flex justify-between items-center mb-4 border-b border-gray-800 pb-2">
                    ...
                    <span className="text-sm text-red-600 font-bold">70/-</span>
                </div>
                    ...
                    {['Brioche', 'Potato'].map((bun) => (
                        <button
                            ...
                            className={`p-4 rounded-xl border transition-all duration-200 flex flex-col items-center gap-2 ${
                                state.bun === bun 
                                ? 'bg-red-600/10 border-red-600 shadow-[0_0_15px_rgba(220,38,38,0.2)]' 
                                : 'bg-gray-800 border-gray-700 hover:border-gray-600'
                            }`}
                        >
                            <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${state.bun === bun ? 'border-red-600' : 'border-gray-500'}`}>
                                {state.bun === bun && <div className="w-2 h-2 rounded-full bg-red-600"></div>}
                            </div>
                            <span className={`font-bold ${state.bun === bun ? 'text-red-600' : 'text-gray-300'}`}>{bun} Bun</span>
                        </button>
                    ))}
                </div>
            </section>
            ...
            <section className="bg-gray-900/50 border border-gray-800 p-5 rounded-2xl animate-in slide-in-from-bottom-4 duration-500 delay-75">
                ...
                <div className="flex items-center justify-between bg-gray-800 p-3 rounded-xl border border-gray-700">
                    ...
                    <button 
                        ...
                        className={`w-12 h-6 rounded-full transition-colors relative ${state.mayo ? 'bg-red-600' : 'bg-gray-600'}`}
                    >
            ...
            <section className={`bg-gray-900/50 border p-5 rounded-2xl animate-in slide-in-from-bottom-4 duration-500 delay-100 ${!hasPatty ? 'border-red-500/50 shadow-[0_0_10px_rgba(220,38,38,0.1)]' : 'border-gray-800'}`}>
                ...
                <div className="mb-6">
                    <h4 className="text-red-600 font-bold mb-3 flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-red-600"></span> Beef Patty (4/- per gram)
                    </h4>
                    ...
                    <div className="flex items-center gap-2 bg-gray-800 p-2 rounded-lg border border-gray-700">
                        <input 
                            ...
                            className="bg-transparent w-20 text-center border-b border-gray-600 text-white focus:outline-none focus:border-red-600"
                        />
                ...
                <div>
                    <h4 className="text-red-600 font-bold mb-3 flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-red-600"></span> Chicken Patty
                    </h4>
                        ...
                        {['Crispy', 'Grilled'].map(type => (
                            <div key={type} className="bg-gray-800 rounded-lg p-3 border border-gray-700">
                                ...
                                <div className="flex gap-2">
                                    {['Small', 'Regular'].filter(size => !(type === 'Crispy' && size === 'Small')).map(size => {
                                        ...
                                        return (
                                            <div key={size} className="flex-1 bg-gray-900 rounded border border-gray-800 p-2 flex justify-between items-center">
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-xs text-red-600 font-bold">{price}/-</span>
                                                </div>
            ...
            <section className="bg-gray-900/50 border border-gray-800 p-5 rounded-2xl animate-in slide-in-from-bottom-4 duration-500 delay-150">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="text-sm text-red-600">60/- per slice</span>
                </div>
                        ...
                        <div className="flex gap-2">
                            {['None', 'Raw', 'Grilled'].map(opt => (
                                <button 
                                    ...
                                    className={`flex-1 py-1.5 rounded text-xs font-bold transition-colors ${state.veggies.onion === opt ? 'bg-red-600 text-white' : 'bg-gray-900 text-gray-400'}`}
                                >
                                    {opt}
                                </button>
                            ))}
            ...
            <section className="bg-gray-900/50 border border-gray-800 p-5 rounded-2xl animate-in slide-in-from-bottom-4 duration-500 delay-300">
                <div className="flex justify-between items-center mb-4 border-b border-gray-800 pb-2">
                    <h3 className="font-bebas text-2xl text-white">6. Add-ons</h3>
                </div>
                        ...
                        <div className="flex gap-2">
                            {['None', 'Raw', 'Grilled'].map(opt => (
                                <button 
                                    ...
                                    className={`flex-1 py-1.5 rounded text-xs font-bold transition-colors ${state.addons.mushrooms === opt ? 'bg-red-600 text-white' : 'bg-gray-900 text-gray-400'}`}
                                >
                                    {opt}
                                </button>
                            ))}
            ...
            <section className="bg-gray-900/50 border border-gray-800 p-5 rounded-2xl animate-in slide-in-from-bottom-4 duration-500 delay-300">
                <div className="flex items-center justify-between bg-gray-800 p-3 rounded-xl border border-gray-700">
                    <div className="flex flex-col">
                        ...
                        <span className="text-xs text-red-600">3 Pieces (250/-)</span>
                    </div>
                    <button 
                        onClick={() => setState(p => ({ ...p, nuggets: !p.nuggets }))}
                        className={`w-12 h-6 rounded-full transition-colors relative ${state.nuggets ? 'bg-red-600' : 'bg-gray-600'}`}
                    >
            ...
            <section className="bg-gray-900/50 border border-gray-800 p-5 rounded-2xl animate-in slide-in-from-bottom-4 duration-500 delay-300">
                <div className="flex justify-between items-center mb-4 border-b border-gray-800 pb-2">
                    ...
                    <span className="text-sm text-red-600">70/- each</span>
                </div>
            ...
            <section className="bg-gray-900/50 border border-gray-800 p-5 rounded-2xl animate-in slide-in-from-bottom-4 duration-500 delay-300">
                ...
                <textarea 
                    ...
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg p-3 text-sm text-white focus:border-red-600 outline-none resize-none h-24"
                />
            </section>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-gray-950 border-t border-gray-800 p-4 z-50 flex items-center justify-center gap-4 shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
             <button
                 ...
                 className="w-full max-w-md bg-red-600 hover:bg-red-500 disabled:bg-gray-800 disabled:text-gray-500 text-white font-bold font-bebas tracking-wide text-xl py-3 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all"
             >
        ...
        {successModalOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                <div className="bg-gray-900 border border-red-600/50 rounded-2xl w-full max-w-sm p-6 relative z-10 shadow-2xl animate-in zoom-in-95">
